...
public class Joystick : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		if (IsTouchingFront && input != 0 && !m_Grounded)
		{
			WallSliding = true;
		}
		else
		...
		if(WallJumping)
		{
			m_Rigidbody2D.velocity = new Vector2(XwallJumpForce * -input, YWallJumpForce);
			Flip();
			playerControls.NumOfJumps *= 0;
		}
		if (Input.GetButton("Jump") && !m_Grounded && !WallSliding)
		{
	...
	public void Move(float move, bool crouch, bool jump, bool hasdoublejumped)
	{
		...
		if(!m_Grounded && hasdoublejumped)
		{
			m_Grounded = false;
			m_Rigidbody2D.velocity = Vector3.zero;
			m_Rigidbody2D.AddForce(new Vector2(0f, m_JumpForce * 1.7f));
		}
	}
